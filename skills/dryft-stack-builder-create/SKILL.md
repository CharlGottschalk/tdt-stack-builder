---
name: dryft-stack-builder-create
description: Create a small standalone Dryft stack, validate it, and prepare draft community submission notes.
---
...
Find the Dryft workspace root from its .dryft/config.json. Read
.dryft/contracts/stack.md and the installed bundle's docs/usage.md and templates
under .dryft/stacks/dryft-stack-builder/. Skill discovery bridges are pointers;
do not resolve templates relative to a bridge or .dryft/skills/ directory.

If the user says “use dui”, read .dryft/skills/dryft-ui/SKILL.md and
.dryft/contracts/ui.md and use the public core UI CLI for missing inputs. Otherwise
offer DUI or chat/TUI when an interview helps, and retain their preference. A
...
2. Inspect the target and applicable parent instructions. Use a new standalone
   directory outside the Dryft workspace; do not copy or modify any linked project
...
3. Read templates/stack.json, templates/skill/SKILL.md, templates/README.md and
   ...
   Derive one skill name as dryft-<stack-id>-<action> (avoid a repeated
   dryft- prefix when the ID already begins with dryft-). For additional skills use explicit
...
4. For requested knowledge, read templates/knowledge/note.md and author a small
   ...
   Never copy brain notes, candidates, transcripts, .dryft state, secrets, agent
...
6. Run dryft stack validate <target> using a safely quoted path. Read actual output
...
7. Summarize files, ID/version, validation result and selected-content SHA256.
   Provide dryft --workspace <workspace> stack install <target> for review.
   ...
   installation to discover new skills. Document /dryft-* names; on Codex use
   $dryft-* or its skill picker if slash invocation is unavailable.
...
through https://stacks.usedryft.com. Include known ID, description, version, author, license, skills
and repository URL if supplied. Record a real source commit only after committing;
never invent a digest or resolved revision, and do not put registry fields into
stack.json. Do not claim that a registry listing exists.
...
For release preparation, offer /dryft-stack-builder-publish. Creation does not
